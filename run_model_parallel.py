...
def run(args, parallel,  generate):        
    ...
    if parallel is not None:
        worker_num, n_workers = map(int, parallel.split("."))
    ...
    for row in tqdm(sample.iloc, total=n):
        ...
        if n_examples == 0:
            ...
            if args.eval_method == "genread":
                ...
                if is_templatedQA:
                    ...
                    for pid in other_pids:
                        for row2 in knowledge[knowledge.prop_id == pid].sample(n=examples_per_template).iloc:
                            # BM25 or contrieverなら...
                            # それ以外なら "Q: {row2.question} A: {row2.obj}" をappend
        ...
        if len(preds) % 100 == 0:
            ...
            worker_str = "" if parallel is None else f"-worker={parallel}"
    ...
    # 自分用に追加
    score = pd.DataFrame({'accuracy': accuracy_score, 'correct': sample.is_correct.sum(),'wrong': len(sample.is_correct) - sample.is_correct.sum()})
    score.to_csv(f"results/score/model={model_name_alias}-input={args.alias}-method={args.eval_method}-shots={n_examples}-n={len(sample)}{'_int8bit' if args.int8bit is True else ''}{worker_str}.csv")
...
def main():
    ...
    for i in range(n_workers):
        p = multiprocessing.Process(target=run, args=(args, '{}.{}'.format(i, n_workers), copy.deepcopy(generate)))
        processes.append(p)
        p.start()
